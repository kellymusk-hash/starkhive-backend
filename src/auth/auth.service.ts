...
import { forgotPasswordDTO, LoginDto, resetPasswordDTO, VerifyEmailDto } from './dtos/auth.dto';
...
export class AuthService {
    ...
    async login(dto: LoginDto): Promise<any> {
        try {
            ...
            const isPasswordValid = await this.utilService.confirmPassword(dto.password, user.password);
            if (!isPasswordValid) {
                throw new UnauthorizedException('Invalid credentials');
            }
    ...
    async resetPassword(dto: resetPasswordDTO): Promise<any> {
        try {
            const user = await this.userRepository.findOne({ where: { resetToken: dto.resetToken } });
